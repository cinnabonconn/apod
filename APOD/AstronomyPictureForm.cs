    ...
    public partial class AstronomyPictureForm : Form
    {
        ...
        private void btnGetForDate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // This will throwa FormatException if the date can't be parsed
                DateTime date = DateTime.Parse(txtDate.Text);

                // Make sure the date is  today or in the pasy
                if (date > DateTime.Today)
                {
                    // Throw FormatException, to be caught in the catch block below
                    throw new FormatException("Date cna't be in the future");
                }

                // And make sure data is june 16, 1995 or later, the date APOD service started
                ...
                //fetch Astronomy picturefor this date
                GetAPOD(date);
            }
            catch(FormatException err)
            {
                // This catch block will handle all the different typesof error
        ...
        private void HandleResponse(APODResponse apodResponse, string error)
        {
            ...
            // Make sure response is an image (not a videoor other media type) before loading
